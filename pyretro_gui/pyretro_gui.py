...
__internal_clock = pygame.time.Clock()
...
def window_render (image_path = None):
    window = app_state.Window

    # clear window
    win_size = window.get_size()
    if image_path is None:
        window.fill(Colors.LIGHT_BG)
    else:
        img = pygame.image.load(image_path).convert_alpha()
        img = pygame.transform.scale(img, win_size)  # redimensionar la imagen al tamaño de la ventana
        window.blit(img, (0,0))
